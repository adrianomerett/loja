...
if ($acao == 'delete-foto-db') {
    ...
    try {
        ...
        $etxra = ROOT_PROCUCTS . "extra" . DS . $img->img;
        $thamb = ROOT_PROCUCTS . "thamb" . DS . $img->img;
        ...
        $deleteimg = $mimg->deleteImage($img->imgid);
        if (!is_integer($deleteimg)) {
            throw new Exception("Erro ao deletar a foto!");
        }
        $retorno['status'] = true;
        $retorno['msg'] = "Foto deletada com sucesso!";
        return App::setJson($retorno);
    } catch (Exception $e) {
...
// Atualizar produto
if ($acao == 'update-products') {
    $retorno = array("status" => false, "msg" => "", "campo" => "", "fotos" => array());
    try {
        $dados = App::getPost('dados');
        $nome = $dados['nome'];
        $descricao = $dados['descricao'];
        ...
        foreach ($fotos as $key => $value) {
            ...
            $resizeextra = $img::resizeImage($pathtmp, $patchthamb, $name, 120, 120);
            if ($resizeextra) {
                $insertimg = $mimg->insertImage(array('idproduto' => $id, 'img' => $name));
                if (!is_integer($insertimg)) {
                    // Deleta a imagem tmp 
                    unlink($pathtmp . $name);
                    continue;
        ...
        $retorno['status'] = true;
        $retorno['msg'] = 'O produto foi atualizado com sucesso!';
        $retorno['fotos'] = $mimg->getImages($id);
        return App::setJson($retorno);
    } catch (Exception $e) {
        $retorno['msg'] = $e->getMessage();
        return App::setJson($retorno);
