...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Descrição
    const descricao = new Quill('#descricao', {
        theme: 'snow',
        modules: {
            toolbar: [
    ...
    descricao.clipboard.dangerouslyPasteHTML(valor_info);
    ...
    informacoes.clipboard.dangerouslyPasteHTML(valor_info_tec);

    // Carrega as fotos do banco no filefotos
    let elementfotosdb = document.getElementsByClassName('col-cad-fotos');
    for (let i of elementfotosdb) {
        let id = i.id.split('-').pop();
        filesfotos[`${id}`] = { 'file': null, 'id': id, 'ext': 'jpg', 'status': false, 'db': 'yes' };
    }
    ...
    document.getElementById('fotos').addEventListener('change', async function () {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            let id = gerarId();
            filesfotos[`${id}`] = { 'file': file, 'id': id, 'ext': ext, 'status': false, 'db': 'none' };
            htmlfoto += `
    ...
    // Salvar produto
    document.getElementById('update-product').addEventListener('click', async function () {
        await updateProduct();
    });
...
async function updateProduct() {
    try {
        ...
        showLoader();
        let req = await api.post('produtos/update-products', { dados: dados });
        showLoader();
        let { status, msg, campo, fotos } = req.data;
        if (status == false) {
            showAlert(msg, 'error');
            if (campo != '') {
        ...
        if (status == true) {
            ...
            // Popular o html das fotos com informações do banco de dados
            let htmlfoto = '';
            filesfotos = {};
            for (let i of fotos) {
                filesfotos[`${i.imgid}`] = { 'file': null, 'id': i.imgid, 'ext': 'jpg', 'status': false, 'db': 'yes' };
                htmlfoto += `
                <div class="col co-sm-12 col-md-2 col-lg-2 col-cad-fotos" id="col-${i.imgid}">
                    <span class="remove-foto-db" id="span-${i.imgid}"><i class="fa-solid fa-trash-can"></i> Remover</span>
                    <div class="ct-foto-upload">
                        <img src="${BASE_URL}/public/upload/produtos/thamb/${i.img}" alt="Foto" id="foto-${i.imgid}">
                    </div>
                </div>
                `;
            }
